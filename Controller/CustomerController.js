...
$("#CustomerManage .updateBtn").click(function () {
    ...
    if (validResult) {
        getAllCustomers()
            .then((customers) => {
                let customerIndex = customers.findIndex((c) => c.custId === UpdateCustomer.custId);
                if (customerIndex >= 0) {
                    updateCustomer(UpdateCustomer);
                    refresh();
                } else {
...
//         });
     getAllCustomers()
         .then((response) => {
             console.log(response); // Log the full response object to inspect its structure

             const customers = Object.values(response); // Adjust this based on the structure
             ...
             if (Array.isArray(customers)) {
                 ...
                 alert("Unexpected data format received. Please contact support.");
             }
         })
         .catch((error) => {
             console.error("Failed to get customers", error);
